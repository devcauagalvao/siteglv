...
const GlobalNetworkBackgroundDark: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Desenhar mapa de pontos discretos (world dots) - versão escura
    const drawWorldDots = () => {
      const dotSize = 1.5;
      const dotColor = "rgba(100, 150, 200, 0.25)";
    ...
    const drawConnections = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (distance < 400) {
            const opacity = (1 - distance / 400) * 0.4;

            // Gradiente para linha (versão escura - cyan/blue)
            const gradient = ctx.createLinearGradient(nodes[i].x, nodes[i].y, nodes[j].x, nodes[j].y);
            gradient.addColorStop(0, `rgba(34, 211, 238, ${opacity * 0.8})`);
    ...
    // Desenhar nós com glow neon (versão azul para dark mode)
    const drawNodes = (time: number) => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        // Glow externo (versão escura - azul)
        const glowGradient = ctx.createRadialGradient(node.x, node.y, 0, node.x, node.y, node.size * 8);
        glowGradient.addColorStop(0, `rgba(59, 130, 246, ${0.4 * glowIntensity})`);
        glowGradient.addColorStop(0.5, `rgba(30, 144, 255, ${0.2 * glowIntensity})`);
        ...
        // Núcleo do nó (versão escura - azul cyan)
        const coreGradient = ctx.createRadialGradient(node.x, node.y, 0, node.x, node.y, node.size);
        coreGradient.addColorStop(0, `rgba(34, 211, 238, 1)`);
        coreGradient.addColorStop(0.7, `rgba(59, 130, 246, ${0.8 * glowIntensity})`);
        coreGradient.addColorStop(1, `rgba(30, 144, 255, 0.3)`);
        ...
        // Anel oscilante (versão escura)
        ctx.strokeStyle = `rgba(34, 211, 238, ${0.3 * glowIntensity})`;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
...
export default GlobalNetworkBackgroundDark;
